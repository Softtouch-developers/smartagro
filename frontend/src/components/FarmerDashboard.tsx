...
export function FarmerDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 pb-24">
      ...
      {/* Content Area */}
      <div>
        {activeTab === 'products' && <ProductListing userType="farmer" key={refreshKey} />}
        {activeTab === 'orders' && <FarmerOrders />}
        {activeTab === 'chat' && <AdvisoryChat />}
      </div>

      {showAddProduct && <AddProductModal onClose={() => setShowAddProduct(false)} onProductCreated={handleProductCreated} />}
    </div>
  );
}